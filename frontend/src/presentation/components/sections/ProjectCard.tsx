    ...
    return (
        <a
            ...
            className={`group block relative rounded-xl overflow-hidden transition-all duration-300 ${
                featured
                    ? 'ring-1 ring-accent/20 shadow-sm shadow-accent/5'
                    : ''
        ...
        >
            <div className="p-3 md:p-4">
                <div className="flex items-start justify-between gap-2 mb-1.5">
                    ...
                    {featured && (
                        <svg className="w-3 h-3 shrink-0 text-accent" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    {technologies.length > 0 && (
                        <span className="flex items-center gap-1">
                            {technologies.slice(0, 2).map((tech, idx) => (
                                <span
                                    key={idx}
                                    className="inline-flex px-1.5 py-0.5 rounded text-[9px] font-medium bg-border/40 text-text-tertiary"
                                >
                                    {tech.name}
                                </span>
                            ))}
                            {technologies.length > 2 && (
                                <span className="text-[9px] text-text-tertiary">+{technologies.length - 2}</span>
                            )}
                        </span>
                    )}
                </div>
